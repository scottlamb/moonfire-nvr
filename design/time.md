...
This document establishes Moonfire NVR's approach for noting the time
associated with recorded video and other data.
...
its camera start time, determined via the following rules:
   ...
   # if there is no camera start time (due to the lack of a RTCP sender
     report), the local start time wins by default.
   # if the camera start time is before 2016-01-01 00:00:00 UTC, the local
     start time wins.
   # if the local start time is before 2016-01-01 00:00:00 UTC, the camera
     start time wins.
